...
void shw3::YaraHelper::initialize()
{
	...
	if (GetModuleInformation(processHandle, this->moduleHandle, &this->moduleInfo, sizeof(this->moduleInfo)) != TRUE) {
		...
		LOG(LL_ERR, "Failed to get module information:", err, " ", shw3::Logger::hex(err, 8));
	}


	LOG(LL_NFO, "Process: base:", shw3::Logger::hex((uint64_t)this->moduleInfo.lpBaseOfDll, 16), " size:", shw3::Logger::hex(this->moduleInfo.SizeOfImage, 16));
