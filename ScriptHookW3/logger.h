...
#ifndef LOG
#define LOG(ll, ...) if (ll <= shw3::Logger::instance().level) {shw3::Logger::instance().write(\
	shw3::Logger::instance().getTimestamp(),\
	" ",\
	shw3::Logger::instance().getLogLevelString(ll),\
	" ",\
	shw3::Logger::instance().getThreadId(),\
	" ",\
	__FILENAME__,\
	" (line ",\
	__LINE__,\
